...
onReady(function () {
  ...
  if (!reduced) {
    ...
    if (hero) {
      ...
      /* Pause loop when hero scrolls out of view */
      var heroVisObs = new IntersectionObserver(function (entries) {
        if (entries[0].isIntersecting) {
          if (!particleRAF) drawParticles();
        } else {
  ...
  /* ==============================================================
     26. CONVERSION TRACKING — fires gtag events on CTA clicks
     ============================================================== */
  document.querySelectorAll('a[href="#booking"]').forEach(function (btn) {
    btn.addEventListener('click', function () {
      if (typeof gtag === 'function') {
        gtag('event', 'cta_click', {
          event_category: 'engagement',
          event_label: btn.textContent.trim().replace(/\s+/g, ' ')
        });
      }
    });
  });
  ...
  if (bookingLink) {
    bookingLink.addEventListener('click', function () {
      if (typeof gtag === 'function') {
        gtag('event', 'booking_click', { event_category: 'conversion' });
      }
    });
  }
